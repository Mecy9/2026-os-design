...
{
  if(__atomic_fetch_sub(&rwlk->readers, 1, __ATOMIC_RELEASE) == 0)
    panic("read_release");
}
...
write_acquire_inner(struct rwspinlock *rwlk)
{
  ...
  do {
    expected = 0;
  } while(!__atomic_compare_exchange_n(&rwlk->writer, &expected, 1, 0,
                                        __ATOMIC_ACQUIRE,
                                        __ATOMIC_RELAXED));
  __atomic_fetch_sub(&rwlk->waiting_writers, 1, __ATOMIC_RELEASE);
...
{
  if(__atomic_exchange_n(&rwlk->writer, 0, __ATOMIC_RELEASE) == 0)
    panic("write_release");
}
...
initrwlock(struct rwspinlock *rwlk)
{
  ...
  __atomic_store_n(&rwlk->writer, 0, __ATOMIC_RELAXED);
  __atomic_store_n(&rwlk->waiting_writers, 0, __ATOMIC_RELAXED);
}
