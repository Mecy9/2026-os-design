...
#ifdef LAB_PGTBL
// Return the level-1 PTE for a 2 MiB-aligned virtual address.
static pte_t *
superwalk(pagetable_t pagetable, uint64 va, int alloc)
{
  if(va >= MAXVA || va % SUPERPGSIZE != 0)
    panic("superwalk");
...
vmprintwalk(pagetable_t pagetable, int level, uint64 base)
{
  for(int i = 0; i < 512; i++){
    ...
    for(int depth = level; depth < 3; depth++)
      printf(" ..");
    printf("%p: pte %p pa %p\n", (void*)va, (void*)pte,
           (void*)PTE2PA(pte));
...
demotesuperpage(pagetable_t pagetable, uint64 va, pte_t *superpte)
{
  ...
  pagetable_t lower = (pagetable_t)kalloc();
  ...
  for(int i = 0; i < 512; i++){
    ...
    memmove(page, (void*)(oldpa + (uint64)i * PGSIZE), PGSIZE);
    lower[i] = PA2PTE(page) | flags;
  }
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < endva; a += sz){
    ...
    if(level == 1){
      uint64 superbase = a & ~((uint64)SUPERPGSIZE - 1);
      ...
      if(demotesuperpage(pagetable, a, pte) < 0)
        panic("uvmunmap: demote");
      sz = 0;
      continue;
    }
    if(level != 0)
      panic("uvmunmap: leaf level");
    if(do_free)
